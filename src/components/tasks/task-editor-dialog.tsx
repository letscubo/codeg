...
function TaskEditorBody({
  ...
  const saveTemplate = async () => {
    ...
    try {
      // A template is reusable across tasks and folders: neither the per-task
      // env overlay (a baked-in sub-agent session key would hand every task
      // made from this template the same session) nor a branch name from one
      // repository (it would only fail the launch in another) may ride along.
      const templateConfig = await buildConfig()
      delete templateConfig.runtime_env
      delete templateConfig.base_branch
