//! Shell 命令 → 人话摘要的解析器。
//!
//! **原样搬自 openai/codex 的 `codex-rs/shell-command/src/parse_command.rs`**
//! (Apache-2.0),只做三处改动:
//!   1. `ParsedCommand` 在本文件内定义(上游在 protocol crate,带 schemars / ts_rs derive,
//!      codeg 不需要那两个依赖);
//!   2. 去掉上游放在文件开头的单元测试(codeg 自己的测试在 tests.rs);
//!   3. 公开 `parse_command`,供 `super::describe` 生成 description。
//!
//! ⚠️ 上游原注释:这段解析很复杂、不要手改;要改就加测试让模型改。
//! 同步上游时整体覆盖本文件的实现部分即可。
...
/// 命令归类结果 —— 与上游 `codex_protocol::parse_command::ParsedCommand` 字段一致。
