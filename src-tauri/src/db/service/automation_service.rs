...
pub async fn automation_for_conversation(
    ...
    let Some(auto) = automation::Entity::find_by_id(run.automation_id)
        .one(conn)
        .await?
    else {
        return Ok(None);
...
mod tests {
    ...
    async fn automation_for_conversation_resolves_through_the_run() {
        ...
        let run = start_run(&db.conn, a.id, "schedule", None)
            .await
            .expect("run");
        ...
        assert_eq!(
            automation_for_conversation(&db.conn, orphan_conv)
                .await
                .expect("lookup"),
            None,
    ...
    async fn reuse_session_is_read_out_of_the_config_blob() {
        ...
        assert!(
            found.reuse_session,
            "reuse_session lost on the way out of config"
        );
    }
